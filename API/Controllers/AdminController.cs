...
public class AdminController(IUnitOfWork unitOfWork, IPaymentService paymentService) : BaseApiController
{
    ...
    public async Task<ActionResult<IReadOnlyList<OrderDto>>> GetOrders([FromQuery] OrderSpecParams orderSpecParams)
    {
        ...
        return await CreatePagedResult(unitOfWork.Repository<Order>(), 
            spec, orderSpecParams.PageIndex, orderSpecParams.PageSize, o => o.ToDto());
    }
    ...
    public async Task<ActionResult<OrderDto>> RefundOrder(int id)
    {
        ...
        if (order is null) return BadRequest("No order with this id");
